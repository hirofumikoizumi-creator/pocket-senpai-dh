...
export function buildSafetyFallbackResponse(query: string): ConsultationResponse {
  return {
    ...
    keywords: [],
    conclusion:
      'この内容は診断・治療などの判断に関わる可能性があるため、本アプリでは回答を控えます。学習用の一般的な確認事項として整理してください。',
    fieldAction:
      '患者さん個別の判断は行わず、まず記録・院内手順・歯科医師または所属医院の方針を確認してください。緊急性を感じる場合も、自己判断せず院内の報告ルートに沿って共有してください。',
    patientTalk:
...
export function buildNoMatchResponse(): ConsultationResponse {
  return {
    ...
    fieldAction:
      '1. 何に困っているかを短く書き出す  2. 患者さんへの説明なのか、手順確認なのかを分ける  3. 判断が必要な部分は歯科医師・先輩・院内マニュアルへ確認する',
